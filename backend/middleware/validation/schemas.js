...
const employeeSchemas = {
  create: Joi.object({
    ...
    employee_number: Joi.string().trim().min(1).max(50).required().messages({
      'string.empty': 'Employee number is required',
      'string.max': 'Employee number cannot exceed 50 characters',
    }),
    monthlyConsumptionValue: Joi.number().integer().min(0).required().messages({
...
const productSchemas = {
  create: Joi.object({
    ...
    price: patterns.price.required().messages({
...
const purchaseSchemas = {
  create: Joi.object({
    ...
    employeeId: patterns.id.required().messages({
...
const purchaseItemSchemas = {
  create: Joi.object({
    productId: patterns.id.required().messages({
      'number.base': 'Product ID must be a number',
      'any.required': 'Product ID is required',
    }),
    purchaseId: patterns.id.required().messages({
...
const idParamSchema = Joi.object({
  id: patterns.id.required().messages({
