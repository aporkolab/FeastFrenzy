      ...
      switch (resourceType) {
        case 'purchase': {
          ...
          if (purchase.userId !== req.user.id) {
            return next(
              createError(403, 'Access denied. You can only access your own purchases.')
            );
          }
          break;
        }
        ...
        case 'employee': {
          ...
          if (employee.userId !== req.user.id) {
            return next(
              createError(403, 'Access denied. You can only access your own profile.')
            );
          }
          break;
        }
        ...
        default:
          return next(createError(500, `Unknown resource type: ${resourceType}`));
      }
