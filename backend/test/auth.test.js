...
describe('Auth API', () => {
  ...
  describe(`POST ${API_BASE}/register`, () => {
    it('should register a new user successfully', async () => {
      ...
      expect(res.body.user).to.have.property('email', validUser.email.toLowerCase());
  ...
  describe(`POST ${API_BASE}/forgot-password`, () => {
    ...
    it('should return success message for existing email', async () => {
      ...
      expect(res.body).to.have.property('message', 'If email exists, reset link sent');
  ...
  describe(`POST ${API_BASE}/reset-password`, () => {
    ...
    it('should return 400 for expired reset token', async () => {
      ...
      const expiredHashedToken = crypto.createHash('sha256').update(resetToken).digest('hex');
      await db.users.update(
        {
          passwordResetExpires: new Date(Date.now() - 60 * 60 * 1000), 
        },
  ...
  describe('Security & Edge Cases', () => {
    it('should not expose password in any response', async () => {
      ...
      expect(JSON.stringify(registerRes.body)).to.not.include(validUser.password);
